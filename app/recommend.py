...
with open(r"C:\Users\saura\OneDrive\Desktop\Data Set\tfidf_vectorizer.pkl", 'rb') as file:
    tfv = pickle.load(file)

tfv_matrix = scipy.sparse.load_npz(r"C:\Users\saura\OneDrive\Desktop\Data Set\tfidf_matrix.npz")

df = pd.read_csv(r"C:\Users\saura\OneDrive\Desktop\Data Set\modified_amazon.csv")
...
def plot_bar_with_dashed_line(df, new_product_df, column, y_label):
    ...
    ax = sns.barplot(data=df, y=column, x='product_name', ci=None, palette='viridis', edgecolor=".2", linewidth=1.5,
                     saturation=0.8, capsize=0.1, errwidth=0.5, dodge=True, width=0.8)
...
def give_rec_w_sentiment(new_product_df):
    ...
    dfw = df[['product_name', 'discounted_price', 'rating', 'rating_count', 'review_content', 'overall_sentiment_score',
              'overall_sentiment_intensity']].iloc[product_indices]
    ...
    dfw = dfw.sort_values('total_weighted', ascending=False)[:5]
...
def recommend():
    if request.method == 'POST':
        ...
        if 'rateC' in request.form:
            rating_count = float(request.form['rateC'])
        else:
            # Handle the case where 'rateC' is not present
            rating_count = 0.0
        sentiment_analysis = int(request.form['sentiment_analysis'])
